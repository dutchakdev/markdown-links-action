...
tslib_1.__exportStar(__nccwpck_require__(6984), exports);
...
async function run() {
    try {
        ...
        if (isNaN(maxDepth) || maxDepth < -1) {
            core.error(`Invalid value for max-depth. It must be a non-negative integer or -1.`);
        ...
        const linkCheckPromises = filesToCheck.map(async (file) => {
            ...
            try {
                ...
                const result = await (0, deadLinks_1.checkLinks)(file, concurrencyLevel);
                core.debug(`Result for file ${file}: ${JSON.stringify(result)}`);
                return result;
            }
            catch (error) {
                if (error instanceof Error) {
                    core.error(`Error while checking links in file ${file}. Error: ${error.message}`);
                }
            ...
            try {
                ...
                for (const deadLink of deadLinks) {
                    ...
                    if (deadLink.file.startsWith(process.env.GITHUB_WORKSPACE || '')) {
                        deadLink.file = deadLink.file.replace(`${process.env.GITHUB_WORKSPACE}/` || '', '');
                        deadLink.link = deadLink.link.replace(`${process.env.GITHUB_WORKSPACE}/` || '', '');
                    }
                }
        ...
        if (createIssue) {
            ...
            const deadLinksIssue = await (0, githubUtils_1.findIssueWithTitle)(octokit, owner, repo, issueTitleEnd);
            try {
                if (deadLinksIssue) {
                    core.debug(`======`);
                    core.debug(`existingIssue: #${deadLinksIssue.number} ${JSON.stringify(deadLinksIssue)}`);
                    core.debug(`======`);
                    await (0, githubUtils_1.createOrUpdateComment)(octokit, owner, repo, deadLinksIssue.number, issueBody, owner);
                    ...
                    await (0, githubUtils_1.createGhIssue)(octokit, owner, repo, `${fullIssueTitle.replace('{n}', deadLinks.length.toString())}`, issueHeader, issueBody, ghAssignees, ghLabels);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.checkLinks = exports.checkWebLink = exports.fileExists = exports.deadLinksToMarkdown = exports.createLinkProcessor = void 0;
const fs_1 = __importDefault(__nccwpck_require__(7147));
const path_1 = __importDefault(__nccwpck_require__(1017));
const stream_1 = __nccwpck_require__(2781);
const axios_1 = __importStar(__nccwpck_require__(8757));
const async_mutex_1 = __nccwpck_require__(4038);
const createLinkProcessor = (file) => {
    let buffer = '';
            ...
            for (const line of lines) {
                ...
                if (linkMatch) {
                    for (const match of linkMatch) {
                        const link = match.match(/href="(.*?)"/)?.[1] || match.match(/\((.*?)\)/)?.[1];
                        if (link) {
                            this.push({ link, file });
                        }
                    }
...
exports.createLinkProcessor = createLinkProcessor;
...
const deadLinksToMarkdown = (deadLinks) => {
    const markdownLines = deadLinks.map(deadLink => `| ${deadLink.file} | ${deadLink.link} | ${renderBadge(deadLink.statusCode)} | ${deadLink.statusEmo} |\n`);
...
const checkWebLink = async (link, file, semaphore) => {
...
exports.checkWebLink = checkWebLink;
const checkLinks = async (file, concurrency = 5) => {
    const deadLinks = [];
    const semaphore = new async_mutex_1.Semaphore(concurrency);
    const linkProcessor = (0, exports.createLinkProcessor)(file);
    return new Promise((resolve, reject) => {
        const stream = fs_1.default.createReadStream(file).pipe(linkProcessor);
        const handleDataEvent = async (linkFilePair) => {
            stream.pause(); // Pause the stream to process this link
            try {
                await semaphore.runExclusive(async () => {
                    if (linkFilePair.link.startsWith('http://') ||
                        linkFilePair.link.startsWith('https://')) {
                        const deadLink = await (0, exports.checkWebLink)(linkFilePair.link, linkFilePair.file, semaphore);
                        if (deadLink) {
                            deadLinks.push(deadLink);
                        }
                    }
                    else if (!linkFilePair.link.startsWith('#')) {
                        const directoryPath = path_1.default.dirname(linkFilePair.file);
                        const relativeFilePath = path_1.default.join(directoryPath, linkFilePair.link);
                        const exists = await (0, exports.fileExists)(relativeFilePath);
                        if (!exists) {
                            deadLinks.push({
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getModifiedFiles = void 0;
const fs_1 = __importDefault(__nccwpck_require__(7147));
const path_1 = __importDefault(__nccwpck_require__(1017));
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.createOrUpdateComment = exports.getDeadLinksComment = exports.findIssueWithTitle = exports.createGhIssue = void 0;
async function createGhIssue(octokit, owner, repo, title, body, comment, assignees, labels) {
...
async function findIssueWithTitle(octokit, owner, repo, titleEnd) {
    ...
    const deadLinksIsssue = issues.data.find((issue) => issue.title.endsWith(titleEnd));
...
async function getDeadLinksComment(octokit, owner, repo, issue_number) {
    if (issue_number === 0) {
        return '';
    }
    ...
    const botComment = comments.data.find(comment => comment.user &&
...
exports.getDeadLinksComment = getDeadLinksComment;
